  ...
  const newsAPI = async () => {
    try {
      const res = await fetch(
        `https://newsapi.org/v2/everything?q=Technology&from=2024-02-01&sortBy=popularity&apiKey=`,
        {
          method: 'GET',
        }
      );
      ...
      if (data.articles && data.articles.length > 0) {
        ...
        const top3Articles = data.articles.slice(0, 3);
                ...
                <div className="mx-auto max-w-2xl lg:max-w-5xl">
                  <div className="mx-auto grid max-w-xl grid-cols-1 gap-y-20 lg:max-w-none lg:grid-cols-2">
                    <div className="flex flex-col gap-16">
                      <h1 className="text-4xl font-display text-white uppercase "><span className="text-xs text-[#DA3028]">get up to date!</span><br />Tech news</h1>
                      {news.map((article, index) => (
                        <Article key={index} title={article.title} description={article.description} time={article.publishedAt} />
                      ))}
                    </div>
                  </div>
                </div>
